...
class LoraUnorderedBatchInfer:
    ...
    def forward(
        ...
        if is_prefill:
            ...
            return self._prefill(batch_size, total_token_num, max_len_in_batch,
                                 input_ids,
                                 b_loc, b_start_loc, b_seq_len, no_lora_compute, target_ids=target_ids)
        else:
            for _ in range(3):
            ...
            return self._decode(batch_size, total_token_num, max_len_in_batch,
                                ...
                                no_lora_compute, no_lora_copy, target_ids=target_ids)

    def _prefill(self, batch_size, total_token_num, max_len_in_batch,
                 input_ids,
                 b_loc, b_start_loc, b_seq_len, no_lora_compute=False, target_ids=None):

        # print("Batch size in prefill: ", batch_size)
    ...
    def _decode(self, batch_size, total_token_num, max_len_in_batch,
                input_ids,
                b_loc, b_start_loc, b_seq_len, no_lora_compute=False, no_lora_copy=False, target_ids=None):

        # print("Batch size in decode: ", batch_size)
    ...
    @final
    def _token_forward(self, input_ids, infer_state, no_lora_compute=False, no_lora_copy=False, target_ids=None):
        cuda_input_ids = input_ids
        cuda_target_ids = target_ids

        # print("Input ids shape: ", cuda_input_ids.shape)
        # print("Target ids shape: ", cuda_target_ids.shape)

        input_embs = self.base_model.pre_infer.token_forward(
            cuda_input_ids, infer_state, self.base_model.pre_post_weight)

        # print("Input embs shape: ", input_embs.shape)

        # conatenate cuda_input_ids and cuda_target_ids and get combined_embeds
        if target_ids is not None:
            cuda_combined_ids = torch.cat([cuda_input_ids, cuda_target_ids], dim=0)
            combined_embeds = self.base_model.pre_infer.token_forward(
                cuda_combined_ids, infer_state, self.base_model.pre_post_weight)
