...
class InferBatch:
    ...
    @classmethod
    @torch.no_grad()
    def init_batch(cls, batch_id, requests, dtype: torch.dtype, device: torch.device, mem_manager: MemoryManager,
                   vocab_size: int):
        ...
        for i, r in enumerate(requests):
            ...
            tokenized_target = r.get('target_id', None)

            input_length = len(tokenized_input)
            input_lengths.append(input_length)
            all_input_ids.append(tokenized_input)
            if tokenized_target is not None:
                all_target_ids.append(tokenized_target)
        ...
        if len(requests) > 1:
            ...
            target_ids = np.concatenate(all_target_ids, dtype=np.int64)
        else:
            input_ids = all_input_ids[0]
            target_ids = all_target_ids[0] if all_target_ids else 0

        # Create tensors on device
        input_ids = torch.tensor(input_ids, dtype=torch.int64, device=device)
        target_ids = torch.tensor(target_ids, dtype=torch.int64, device=device)
    ...
    def free_self(self):
        ...
        for idx in range(len(self)):
            remove_index.append(self.nopad_b_loc[idx,
    ...
    def filter(self, request_ids: List[int]):
        ...
        nopad_b_loc[:, 0: (nopad_max_len_in_batch - 1)] = self.nopad_b_loc[indices,
                                                          (self.nopad_max_len_in_batch - 1) - (
                                                                      nopad_max_len_in_batch - 1): (
                                                                      self.nopad_max_len_in_batch - 1)]
        adapter_dirs = []
        for i, request_id in enumerate(request_ids):
            idx = self.requests_idx_mapping[request_id]
    ...
    def merge(cls, batch1, batch2):
        ...
        for i, batch in enumerate(batches):
            ...
            nopad_b_loc[start_index: end_index,
            nopad_max_len_in_batch - batch.nopad_max_len_in_batch: nopad_max_len_in_batch -
                                                                   1] = batch.nopad_b_loc[:,
        ...
        nopad_b_loc[:, nopad_max_len_in_batch - 1] = nopad_total_token_num - \
                                                     new_batch_size + torch.arange(0, new_batch_size, dtype=torch.int32,
                                                                                   device='cuda')
        return InferBatch(
